...
# địa chỉ giao hàng trước khi đặt hàng
@login_required(login_url='customerlogin')
def customer_address_view(request):
    # Cái này để kiểm tra xem sản phẩm có trong giỏ hàng hay không
    # nếu không có trong giỏ hàng sẽ không hiển thị mẫu địa chỉ
    product_in_cart = False
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        if product_ids != "":
            product_in_cart = True
    # cho quầy trong giỏ hàng
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        counter = product_ids.split('|')
        product_count_in_cart = len(set(counter))
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            # ở đây lấy email, địa chỉ, di động tạm thời điểm đặt hàng
            # chúng tôi không lấy nó khỏi bảng thanh toán khách hàng vì
            # những thứ này có thể thay đổi
            email = addressForm.cleaned_data['Email']
            mobile = addressForm.cleaned_data['Mobile']
            address = addressForm.cleaned_data['Address']
            # để hiển thị tổng giá trên trang thanh toán ..... truy cập id từ cookie sau đó tìm nạp giá sản phẩm từ db
            total = 0
            if 'product_ids' in request.COOKIES:
                product_ids = request.COOKIES['product_ids']
                if product_ids != "":
